...
const citations=ids=>{
  ...
  (ids||[]).forEach((id,index)=>{
    ...
    if(!evidence)return;
    const button=node('button',`证据 ${index+1} · 书内 ${evidence.printed_page_number} 页 · PDF ${evidence.source_pdf_page_number} 页`);
...
const renderReport=()=>{
  ...
  const children=[];
  const notice=node('blockquote','基于程序异常判定和书内检索证据生成，不构成诊断、治疗或用药建议。');notice.className='report-notice';children.push(notice);
...
$('#report-form').addEventListener('submit',async event=>{event.preventDefault();$('#status').textContent='分析中';$('#rule-status').textContent='正在检索书内证据并生成报告';$('#analysis-result').replaceChildren(node('p','请稍候…'));try{const response=await fetch('/api/report-generation',{method:'POST',headers:{'Content-Type':'application/json'},body:$('#report-json').value});const data=await response.json();if(!response.ok)throw Error(data.detail||'请求失败');reportState=data;setReportMode('report');$('#rule-status').textContent='分析完成';$('#status').textContent='完成'}catch(error){reportState=null;$('#analysis-result').replaceChildren(node('p',reportError(error.message)));$('#rule-status').textContent='分析失败';$('#status').textContent='失败'}});
